...
bool IsLocalhost(const GURL& url) {
    ...
    std::string host_lc = base::ToLowerASCII(url.host());
    if (host_lc.find("localhost") != std::string::npos)
      return true;
  
    // Standard Chromium check for loopback/localhost forms
...
bool RequestIsSafeToServe(const net::HttpServerRequestInfo& info,
  ...
  bool is_host_local = is_host_set && IsLocalhost(host_url);
